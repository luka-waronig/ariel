...
class ResultAggregator:
        ...
        for variant in self.brain_variants:
            ...
            for run in range(1, 4):  # runs 1, 2, 3
                csv_file = os.path.join(
                    self.results_dir, f"fitness_progression_{variant}_{run}.csv"
                )
    ...
    def plot_brain_variant(self, variant: str, save_plot: bool = True) -> None:
        ...
        aligned_means = np.array(
            [df["mean"].iloc[:min_gens].values for df in self.data[variant]]
        )
        ...
        if (
            variant in ["selfadaptive", "uniform"]
            and "nobrain" in self.data
            and self.data["nobrain"]
        ):
            # Calculate baseline statistics
            baseline_min_gens = min(len(df) for df in self.data["nobrain"])
            baseline_aligned_means = np.array(
                [
                    df["mean"].iloc[:baseline_min_gens].values
                    for df in self.data["nobrain"]
                ]
            )
            baseline_generations = (
                self.data["nobrain"][0]["gen"].iloc[:baseline_min_gens].values
            )
            ...
            plt.plot(
                baseline_generations[:common_gens],
                baseline_mean[:common_gens],
                color="red",
                linewidth=2,
                linestyle="--",
        ...
        plt.ylabel("Fitness")
        plt.title(
            f'Fitness Evolution - {variant.replace("_", " ").title()} Brain vs Baseline'
        )
        plt.legend()
        ...
        if save_plot:
            plt.savefig(
                f"{self.results_dir}/aggregated_{variant}_fitness.png",
                dpi=300,
                bbox_inches="tight",
            )
    ...
    def print_summary_statistics(self) -> None:
        ...
        for variant in self.brain_variants:
            ...
            for i, df in enumerate(self.data[variant]):
                ...
                print(
                    f"  Run {i+1}: Final fitness = {final_fitness:.4f}, "
                    f"Max fitness = {max_fitness:.4f}"
                )

            if final_fitnesses:
                print(
                    f"  Average final fitness: {np.mean(final_fitnesses):.4f} ± {np.std(final_fitnesses):.4f}"
                )
                print(
                    f"  Average max fitness: {np.mean(max_fitnesses):.4f} ± {np.std(max_fitnesses):.4f}"
                )
